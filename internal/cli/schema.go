...
var commandSchemas = map[string]SchemaCommand{
	...
	"scan": {
		...
		Flags: []SchemaFlag{
			...
			{Name: "with-meta", Type: "bool", Default: "false", Description: "fetch EXIF metadata (GPS, taken date, dimensions); required for country/date/duration filters; first run ~30-60s, cached 7 days"},
			{Name: "country", Type: "string", Default: "", Description: "keep items whose GPS resolves to this country or region, e.g. China, JP, Asia; auto-enables --with-meta"},
	...
	"backup": {
		...
		Flags: []SchemaFlag{
			...
			{Name: "country", Type: "string", Default: "", Description: "back up items from a specific country/region, e.g. Japan, CN; auto-enables --with-meta"},
