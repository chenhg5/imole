...
async function download(url) {
  console.log(`[imole] Downloading ${NAME} ${VERSION} for ${process.platform}/${process.arch}...`);
  try {
    const data = await fetch(url);
    console.log(`[imole] Downloaded ${(data.length / 1024 / 1024).toFixed(1)} MB`);
    return data;
  } catch (err) {
    throw new Error(
      `[imole] Could not download binary.\n` +
        `  ${err.message}\n` +
...
async function main() {
  ...
  if (fs.existsSync(binaryPath)) {
    try {
      ...
      if (out.includes(expectedVer)) {
        console.log(`[imole] Binary ${VERSION} already installed, skipping.`);
        return;
      }
      console.log(`[imole] Existing binary is outdated, upgrading to ${VERSION}...`);
      fs.unlinkSync(binaryPath);
    } catch {
      console.log(`[imole] Replacing existing binary with ${VERSION}...`);
      fs.unlinkSync(binaryPath);
    }
  }
  ...
  if (platform === "darwin") {
    try {
      ...
      console.log(`[imole] Removed macOS quarantine attribute`);
    } catch {
      // xattr fails if the attribute doesn't exist, which is fine
    }
  }

  console.log(`[imole] Installed to ${binaryPath}`);
}
